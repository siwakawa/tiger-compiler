...
| fijaNONE((name, TArray((TTipo(s,r as ref(NONE))), u))::t) env =
	(case tabBusca(s, env) of
	NONE => raise Fail "error interno 666+1"
	| SOME ras => (r := SOME ras; fijaNONE t env))
| fijaNONE((name, TRecord(lf, u))::t) env =
	let	fun busNONE(s, TTipo(tipo, r as ref(NONE)), _) =
			(r := SOME(tabSaca(tipo, env)) handle _ => raise noExiste)
		| busNONE _ = ()
