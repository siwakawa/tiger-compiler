...
struct
    ...
    fun format saytemp =
        ...
        in fn OPER{assem,dst,src,jump=NONE} => speak(assem,src,dst,nil) ^ "\n"
            | OPER{assem,dst,src,jump=SOME j} => speak(assem,src,dst,j) ^ "\n"
	        | LABEL{assem,...} => assem ^ "\n"
	        | MOVE{assem,dst,src} => speak(assem,[src],[dst],nil) ^ "\n"
        end
