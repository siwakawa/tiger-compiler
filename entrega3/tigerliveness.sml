structure tigerliveness :> tigerliveness = struct
    ...
    fun get_liveness_maps(FGRAPH(fgraph)) = let val nodes = tigergraph.nodes(#control fgraph)
        ...
        fun fold_algo'((g, n) : node, (ins, outs, keep_iterating)) = let
            val control = #control fgraph
            ...
            fun union_fold((node, set)) = let 
                val ins_node = case Splaymap.peek(ins, node) of
                    SOME(s) => s
                    | NONE => Splayset.empty(String.compare)
...
(*          val _ = Splaymap.app(fn(n, set)=>print("Node " ^Int.toString(n) ^", Outs: "^ Int.toString(Splayset.numItems(ins_n'))^ "\n")) outs*)
        ...
        fun iterate_until_solution(ins, outs) = let 
            val (ins', outs', keep_iterating) = iterate_algo_once(ins, outs)
    ...
    fun getLiveMap(fg as FGRAPH(flowgraph)) = let 
        val (ins, outs) = get_liveness_maps(fg)
    ...
    fun interferenceGraph(fg as FGRAPH(flowgraph)) = let 
        val liveMap = getLiveMap fg
      (*val _ = Splaymap.app(fn(n, set)=>print("Node " ^Int.toString(n) ^", lives: "^ Int.toString(Splayset.numItems(set))^ "\n")) liveMap*)
...
(*      val _ = Splayset.app(fn(t) => print(t ^", ")) temps_set*)
        ...
        val moves = List.map (fn((g, fg_node)) => if ismove(fg_node) then
            (if (length(getdefs(fg_node)) <> 1) orelse (length(getuses(fg_node)) <> 1) then
                raise Fail "Internal error: Move instruction with #uses/=1 or #defs /=1"
                  ...
                  val from = hd(getuses(fg_node))
                  val to = hd(getdefs(fg_node))
                in [(from, to)]
                end)
            else []) fg_nodes
        ...
        (* moveFun matches a node in the interference graph to a pair (from, to) of temps representing a move instruction *)
        fun movesFun(temp) = Splayset.addList(Splayset.empty(moveOrder), (List.filter (fn((f,t)) => temp=t orelse temp=f) moves'))
        ...
        fun addIntEdges((fg, fg_node)) = let 
            val defs = getdefs(fg_node)
     ...
     in (IGRAPH({graph=igraph, tnode=tnode, gtemp=gtemp, moves=movesFun}), liveMap_fun) 
     end

    fun show(IGRAPH(igraph)) = let 
        val ns = nodes (#graph igraph)
